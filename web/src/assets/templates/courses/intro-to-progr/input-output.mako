...
<%self:lesson>
  ...
  <p>Here comes the big moment. Until now we talked to Python only in the terminal and sent only individual commands to be evaluated. But now that we can work with variables and functions we can create our first program.</p>
...
print(price)
  ...
  <p>This is a simple program that will calculate the price of a ticket in Czech crowns for the exchange rate 25 CZK for 1 Euro. We will run this program with the following command:</p>
...
<%self:exercises>
  ...
  <%self:exrc title="Simple Output">
    ...
    <ol>
      ...
      <li>
        <p>Modify this program so that you put a name of some theatre play into variable <var>name</var> and a time of the performance into variable <var>time</var>. Using the <code>print()</code> function display one line showing the name and the time of the play on the screen like this:</p>
  ...
  <%self:exrc title="Simple Input">
    ...
    <ol>
      <li>Write a program called <code>name.py</cpde> which will obtain user's first and last names using the <code>input()</code> function. Print them out on the screen similar to how you did in previous excercise.</li>
  ...
  <%self:exrc title="Dice Rolls">
    <p>Write a program called <code>dice.py</code> which will simulate rolling two six-sided dice. Its output will be the sum of the two rolls.</p>

    <p><strong>Hint:</strong></p>
    <ul>
      <li>To generate a random number use the <code>randint()</code> function from the <code>random</code> module like this: <code>random.randint()</code>.</li>
      <li>
        <p>If you want to use the <code>random</code> module you have to import it first by adding the following command at the very top of your program:</p>
  ...
  <%self:exrc title="Number Generator">
    <p>Write a program called <code>generator.py</code> which will ask the user for two integers - the lower and upper bounds - and print out a random integer within those bounds.</p>
  </%self:exrc>
</%self:exercises>

<%self:lesson>
  <h2>Single Line Input</h2>
  <p>You've probably noticed that the <code>print()</code> function always starts on a new line. But sometimes we want an output without moving to a new line after each <code>print()</code> so that we can continue the output on the same line.</p>
</%self:lesson>
